...
describe('Circle', () => {
  describe('print', () => {
    it('기본값으로 ^FO0,0\\&^GC{diameter},1,B^FS 생성', () => {
      ...
      expect(Circle.print(el, defaultLabelContext)).toBe('^FO0,0\\&^GC50,1,B^FS');
    });
    ...
    it('fieldOrigin, thickness, lineColor 지정 시 올바른 ZPL 생성', () => {
      ...
      expect(Circle.print(el, defaultLabelContext)).toBe('^FO10,20\\&^GC100,5,B^FS');
    });
    ...
    it('채워진 원 (thickness >= diameter) 허용', () => {
      ...
      expect(Circle.print(el, defaultLabelContext)).toBe('^FO0,0\\&^GC50,50,B^FS');
    });
  });
  ...
  describe('print - 검증', () => {
    it('diameter < 3 시 에러 throw', () => {
      ...
      expect(() => Circle.print(el, defaultLabelContext)).toThrow(
