...
모든 테스트 파일 상단에 동일한 `defaultContext`를 선언한다.
...
```ts
expect(() => Line.print(el, defaultContext)).toThrow('length는 1 이상이어야 합니다');
```
...
import { Barcode, type ZplElementContext } from '@zpl-kit/react-zpl';
...
describe('Barcode', () => {
  describe('print', () => {
    it('code128 기본값 — ^FO, ^BC, ^FD, ^FS 순서로 생성', () => {
      ...
      expect(Barcode.print(el, defaultContext)).toBe('^FO10,20\\&^BCN,60,Y,N,N^FDABC^FS');
    });
    ...
    it('qr — ^BQ 명령어와 ^FD에 QA, 프리픽스 포함', () => {
      ...
      const zpl = Barcode.print(el, defaultContext);
  ...
  describe('print - 검증', () => {
    it('value가 빈 문자열이면 에러 throw', () => {
      ...
      expect(() => Barcode.print(el, defaultContext)).toThrow('value는 비어있을 수 없습니다');
    });

    it('ean13에 13자리가 아닌 값을 전달하면 에러 throw', () => {
      const el = createElement(Barcode, { type: 'ean13', value: '123' });
      expect(() => Barcode.print(el, defaultContext)).toThrow('EAN-13은 13자리 숫자여야 합니다');
    });
  });
